...
- 支持本地分组管理
- 支持启用、禁用、编辑、删除账号
- 支持账号级访问出口配置，可选择直连或自定义 HTTP/HTTPS 代理
- 支持单账号和批量健康检查
...
- 命令面板执行的一键签到、批量健康检查等动作也会复用同一套刷新链路，但不会触发消息推送
- 如果你升级后端后页面行为仍是旧版本，请同步重新构建并发布前端

### 账号代理说明

- 全局代理配置已移除，账号访问出口只支持“直连服务器出口”和“自定义代理”
- 自定义代理仅作用于该账号访问目标平台的请求
- 账号验证、签到、Token 同步、模型列表、分组查询、状态查询等都会走该账号的自定义代理
- 目前支持 `http://` 和 `https://` 代理地址
- 如代理需要认证，可填写完整地址，例如 `http://user:password@127.0.0.1:7890`
- 旧版本账号的 `global` 代理模式会在启动迁移或备份导入时自动转换为 `direct`

## 推送渠道配置

### 推送规则

- 先在「系统设置」的「推送渠道」中新增并启用渠道
- 定时签到推送在「系统设置」的「常规配置」中单独开启，并选择接收汇总的渠道
- 定时签到和定时重试只发送一条全局汇总，内容包含本次账号数、成功数、失败数和签到奖励
- 手动单账号签到、手动批量签到、手动健康检查不会发送消息推送
- 账号编辑里的“健康告警渠道”只用于定时健康检查告警，不用于签到推送
- 渠道被禁用或删除后，会自动从全局签到推送配置中移除
